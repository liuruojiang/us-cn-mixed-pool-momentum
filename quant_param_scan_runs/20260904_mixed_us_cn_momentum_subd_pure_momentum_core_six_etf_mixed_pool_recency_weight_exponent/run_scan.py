...
EXPONENTS = (0.0, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 2.0)
WINDOWS = {
    "full_sample": "full",
    "10Y": "last_10y",
    "5Y": "last_5y",
...
def exponent_label(exponent: float) -> str:
    return f"{exponent:g}"


def candidate_label(exponent: float) -> str:
    return f"weight_power_{exponent_label(exponent).replace('.', 'p')}"


def scorer_for(exponent: float):
    def weighted_slope_score_and_r2(window: pd.Series) -> tuple[float, float]:
        values = window.dropna().astype(float)
        if (
            len(values) != subd.LOOKBACK
            or (values <= 0).any()
        ...
        weights = np.power(np.arange(1, len(y) + 1, dtype=float), exponent)
        ...
        annual_log_return = float(slope) * subd.TRADING_DAYS
        if not math.isfinite(annual_log_return) or annual_log_return > math.log(sys.float_info.max):
            return math.nan, r2
        return math.expm1(annual_log_return), r2

    return weighted_slope_score_and_r2
...
def main() -> None:
    ...
    config = subd.RunConfig(
        ...
        end_date=end,
        output_tag="recency_weight_exponent_scan",
        target_vols=(),
    ...
    original_scorer = subd.weighted_slope_score_and_r2
    try:
        for exponent in EXPONENTS:
            subd.weighted_slope_score_and_r2 = scorer_for(exponent)
            name = candidate_label(exponent)
                ...
                row.update(
                    {
                        "candidate": name,
                        "segment": segment,
                        "weight_power": exponent_label(exponent),
                        "lookback": 25,
                        "score_max": 5.0,
                        "r2": "off",
                        "switch_buffer": 1.0,
        ...
        subd.weighted_slope_score_and_r2 = original_scorer

    long = pd.DataFrame(rows)
    long.to_csv(RUN_DIR / "scan_summary.csv", index=False)
    wide = long[["candidate", "weight_power"]].drop_duplicates().set_index("candidate")
    for metric_name in (
        "ann_return",
        "ann_vol",
        "sharpe_repo",
    ...
    baseline = wide.loc[wide.weight_power == "1"].iloc[0]
    ...
    poe_curve = poe.run_staged_entry(
        prices,
        config,
        poe.EntryCase("weight_power_1", "full_entry", 1.0),
        None,
        1.0,
    ...
    for metric_name in ("return", "nav", "turnover", "fraction_before"):
        parity_rows.append(
            {
                "check": "weight_power_1_runner_vs_poe",
                "metric": metric_name,
    ...
    meta["parity_check"] = {
        "current_power_1_vs_accepted_base_and_runner_vs_poe_nav_max_abs_diff": nav_diff,
