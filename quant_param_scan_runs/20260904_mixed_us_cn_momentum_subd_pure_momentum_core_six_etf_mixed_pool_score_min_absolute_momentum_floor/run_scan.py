...
CAPS = (-math.inf, -0.2, -0.1, -0.05, 0.0, 0.05, 0.1, 0.2, 0.5)
WINDOWS = {
    "full_sample": "full",
    "10Y": "last_10y",
    "5Y": "last_5y",
...
def candidate_label(cap: float) -> str:
    return f"score_min_{cap_label(cap).replace('.', 'p')}"
...
def main() -> None:
    ...
    config = subd.RunConfig(
        ...
        end_date=end,
        output_tag="score_min_pure_momentum_scan",
        target_vols=(),
    ...
    wide = long[["candidate", "score_min"]].drop_duplicates().set_index("candidate")
    metric_names = (
        "ann_return",
        "ann_vol",
        "sharpe_repo",
    ...
    baseline = wide.loc[wide.score_min == "0"].iloc[0]
    ...
    try:
        ...
        poe_curve = poe.run_staged_entry(
            prices,
            config,
            poe.EntryCase("score_min_0", "full_entry", 1.0),
            None,
            1.0,
    ...
    for metric_name in ("return", "nav", "turnover", "fraction_before"):
        parity_rows.append(
            {
                "check": "score_min_0_runner_vs_poe",
                "metric": metric_name,
    ...
    meta["parity_check"] = {
        "current_score_min_0_vs_accepted_base_and_runner_vs_poe_nav_max_abs_diff": nav_diff,
