...
def main() -> None:
    ...
    config = subd.RunConfig(
        ...
        output_tag="r2_extended_clean_base",
        target_vols=(),
    ...
    prior_long = pd.read_csv(PRIOR_DIR / "scan_summary.csv")
    overlap = (0.10, 0.15, 0.20, 0.25, 0.30, 0.40, 0.50)
    parity_rows: list[dict[str, object]] = []
    for threshold in overlap:
        prior_label = f"r2_{threshold:.2f}_buffer_1.00"
        current_label = label_for(threshold)
        for segment in WINDOW_NAMES.values():
            old = prior_long[(prior_long["candidate"] == prior_label) & (prior_long["segment"] == segment)].iloc[0]
            new = long[(long["candidate"] == current_label) & (long["segment"] == segment)].iloc[0]
            for metric in ("ann_return", "ann_vol", "sharpe_repo", "max_dd", "turnover_total", "cost_total"):
                parity_rows.append({
                    "check": "overlap_metric",
                    "threshold": threshold,
                    "segment": segment,
                    "metric": metric,
                    "abs_diff": abs(float(old[metric]) - float(new[metric])),
                })
    for threshold, prior_name in ((None, "r2_off_buffer_1.00"), (0.20, "r2_0.20_buffer_1.00")):
        old = pd.read_csv(PRIOR_DIR / "daily_outputs" / f"{prior_name}.csv.gz")
        new = curves[threshold].reset_index()
        for metric in ("return", "nav"):
            parity_rows.append({
                "check": "daily_curve",
                "threshold": "off" if threshold is None else threshold,
                "segment": "full",
                "metric": metric,
                "abs_diff": float(np.max(np.abs(old[metric].to_numpy(float) - new[metric].to_numpy(float)))),
            })
    ...
    for threshold in (None, 0.0, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.50, 0.70, 0.90):
    ...
    meta["parity_check"] = {
        "overlap_thresholds": list(overlap),
