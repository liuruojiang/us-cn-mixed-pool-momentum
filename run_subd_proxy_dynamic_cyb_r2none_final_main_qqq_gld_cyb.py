...
DEFAULT_TAG = "subd_proxy_dynamic_cyb_r2none_final_main_qqq_gld_cyb_2007_2026_20260702"

REDUCED_CORE_ASSETS = {
    "QQQ": "NASDAQ_QQQ_ADJ_CLOSE_PROXY",
    "GLD": "GOLD_GLD_ADJ_CLOSE_PROXY",
}
REDUCED_PROXY_ASSETS = {
    **REDUCED_CORE_ASSETS,
...
def patched_three_asset_pool() -> Iterator[None]:
    original_core = dict(layer0.CORE_ASSETS)
    original_proxy = dict(layer0.PROXY_ASSETS)
    try:
        layer0.CORE_ASSETS.clear()
        layer0.CORE_ASSETS.update(REDUCED_CORE_ASSETS)
        layer0.PROXY_ASSETS.clear()
        layer0.PROXY_ASSETS.update(REDUCED_PROXY_ASSETS)
        yield
    finally:
        layer0.CORE_ASSETS.clear()
...
def build_oos_continuous(
    ...
    mode = "oos_2017_2026_continuous_state_from_2007_three_asset_pool"
    metrics = pd.DataFrame([final_oos.summarize_curve(curve, metric_index, mode)])
    yearly = final_oos.yearly_returns(curve, metric_index, mode)
    daily = final_oos.daily_output(curve, mode, metric_index)
    return metrics, yearly, daily, sources.assign(mode=mode)
...
def run_three_asset_pool(start_date: pd.Timestamp, split_date: pd.Timestamp, end_date: pd.Timestamp, tag: str) -> None:
    started = time.time()
    output_dir = Path("outputs") / tag
    output_dir.mkdir(parents=True, exist_ok=True)

    with patched_three_asset_pool():
        train_end = split_date - pd.Timedelta(days=1)
        train = build_slice(
            "insample_2007_2016_three_asset_pool",
            start_date,
            train_end,
        )
        oos_reset = build_slice(
            "oos_2017_2026_standalone_reset_three_asset_pool",
            split_date,
            end_date,
        )
    ...
    meta = {
        ...
        "command": (
            "python run_subd_proxy_dynamic_cyb_r2none_final_main_qqq_gld_cyb.py "
