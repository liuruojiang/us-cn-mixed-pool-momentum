# poe: name=SubD-Six-ETF-Naive-V13
...
V13_SCENARIO = "v1_3_naive_score_0p5_5p5_r2_0p25"
...
LIVE_CACHE_STALE_IF_ERROR_MAX_AGE = LIVE_QUOTE_MAX_AGE.to_pytimedelta()
...
def _sina_kline_symbol(code: str) -> str:
    ticker, suffix = code.split(".")
    if suffix == "SZ":
        return f"sz{ticker}"
    if suffix == "SH":
...
def _load_sina_raw_one_close(code: str, end_date: pd.Timestamp) -> pd.Series:
    ...
    params = {
        "symbol": _sina_kline_symbol(code),
        "scale": "240",
        "ma": "no",
...
def _load_public_close_with_per_code_fallback(codes: list[str], end_date: pd.Timestamp) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for code in codes:
        ...
        # Raw helpers remain diagnostic-only; do not fetch data the formal path rejects.
...
def load_close(config: RunConfig) -> tuple[pd.DataFrame, pd.DataFrame]:
    codes = list(ASSETS)
    prices, sources = _load_public_close_with_per_code_fallback(codes, config.end_date)
    diagnostic = sources["adjustment"].astype(str).str.lower() != ADJUSTMENT_QFQ
    if diagnostic.any():
        rejected = ", ".join(
            f"{row.code}:{row.adjustment}"
            for row in sources.loc[diagnostic].itertuples(index=False)
        )
        raise RuntimeError(
            "Raw/unadjusted history is diagnostic-only and cannot enter the formal V1.3 path: "
            + rejected
        )
...
def _recompute_final_exposure_nav(
    ...
    try:
        hard_cap = math.inf if max_lev is None else float(max_lev)
    except (TypeError, ValueError) as exc:
        raise ValueError("max_lev hard cap must be a finite nonnegative number") from exc
    if max_lev is not None and (not math.isfinite(hard_cap) or hard_cap < 0.0):
        raise ValueError("max_lev hard cap must be a finite nonnegative number")
    ...
    for row_number, idx in enumerate(out.index):
        ...
        # the rebalance. A coincident policy trade must not be double-counted.
...
def build_curves(
    ...
    """V1.3: direct full-entry core; disabled overlays must never change the ledger."""
    if config.max_lev != 1.0 or config.target_vols:
        raise ValueError("Six-ETF V1.3 requires max_lev=1 and no target-vol overlays")
    if not np.isfinite(config.one_way_cost) or not 0 <= config.one_way_cost < 0.5:
    ...
    if prices.empty or not prices.index.is_unique or not prices.index.is_monotonic_increasing:
        raise ValueError("Prices must have a nonempty, unique, sorted date index")
    if set(ASSETS) - set(prices.columns):
        raise ValueError("Missing required six-ETF price columns")
    if (prices[list(ASSETS)].le(0) | np.isinf(prices[list(ASSETS)])).any().any():
        raise ValueError("Observed prices must be finite and positive")
    flags = _validate_price_ffill_flags(prices, price_ffill_flags)
    curve = run_staged_entry(
        prices, config, EntryCase("full_entry", "full_entry", INITIAL_ENTRY_FRACTION),
        R2_THRESHOLD, SWITCH_BUFFER, price_ffill_flags=flags,
    )
    # Adapter fields are display-only. Do not invoke drift/vol/overheat recomputation:
    # it would change the frozen core's costs and net returns.
    curve.insert(0, "version", VERSION)
    curve["scenario"] = V13_SCENARIO
    curve["exposure_effective"] = curve["fraction_before"]
    curve["final_exposure_after_overheat"] = curve["holding_fraction"]
    curve["drifted_exposure_before_trade"] = curve["fraction_before"]
    curve["rebalance_delta"] = curve["holding_fraction"] - curve["fraction_before"]
    switched = curve["position_before"].ne(curve["position"])
    curve["buy_delta"] = np.where(switched, curve["holding_fraction"], 0.0)
    curve["sell_delta"] = np.where(switched, curve["fraction_before"], 0.0)
    for field in ("weight", "target_vol_scale_effective", "target_vol_scale_next",
                  "overheat_scale", "overheat_scale_effective", "overheat_scale_next"):
        curve[field] = 1.0
    for field in ("overheat_on", "overheat_on_effective", "overheat_triggered",
                  "overheat_recovered", "overheat_feature_missing"):
        curve[field] = False
    curve["overheat_recovery_mode"] = ""
    curve["base_position_before"] = curve["actual_position_before"] = curve["position_before"]
    curve["base_position_next"] = curve["actual_position_next"] = curve["position"]
    curve["actual_entry_state"] = np.where(curve["position"].eq("CASH"), "CASH", "FULL_POSITION")
    curve["actual_pending_target"] = None
    curve["actual_pending_days"] = 0
    curve["actual_staged_initial"] = False
    curve["actual_fill_on_down_day"] = False
    if not np.allclose(curve["buy_delta"] + curve["sell_delta"], curve["turnover"], atol=1e-12):
        raise ValueError("V1.3 trade-leg and turnover mismatch")
    return [curve]
...
def _build_config(end_date=None) -> RunConfig:
    ...
    return RunConfig(
        source="akshare_em_qfq", one_way_cost=ONE_WAY_COST,
...
def _normalize_daily(daily: pd.DataFrame) -> pd.DataFrame:
    ...
    out = out[out["scenario"].astype(str) == V13_SCENARIO].copy()
    out = out.sort_values("date").reset_index(drop=True)
    if out.empty:
        raise poe.BotError("未找到 SubD six-ETF v1.3 日度输出。")
    return out


def _build_v13_daily(
    end_date=None,
...
def _call_build_v13_daily(
    end_date: pd.Timestamp,
    data_state: str,
    now: datetime,
) -> tuple[pd.DataFrame, str]:
    params = inspect.signature(_build_v13_daily).parameters
    ...
    if accepts_kwargs or "now" in params:
        kwargs["now"] = now
    return _build_v13_daily(**kwargs)
    ...
    if force_refresh:
        ...
        try:
            ...
            if data_state == "live":
                ...
                if cache_age < timedelta(0) or cache_age > LIVE_CACHE_STALE_IF_ERROR_MAX_AGE:
                    raise
...
def _daily_returns_for_window(sub: pd.DataFrame) -> pd.Series:
    if "return" in sub.columns:
        ...
        if ret.isna().any():
            ...
            raise poe.BotError(
                f"missing return inside performance window: {missing_dates}; return must be finite"
            )
        ...
        if bankrupt.any():
            bad_dates = ", ".join(
                pd.Timestamp(value).date().isoformat()
                for value in sub.loc[bankrupt, "date"].iloc[:6]
            )
    ...
    if nav.isna().any():
        ...
        raise poe.BotError(
            f"missing nav inside performance window: {missing_dates}; nav must be finite"
        )
    ...
    if nonpositive.any():
        bad_dates = ", ".join(
            pd.Timestamp(value).date().isoformat()
            for value in sub.loc[nonpositive, "date"].iloc[:6]
        )
...
def _wealth_from_returns(ret: pd.Series) -> pd.Series:
    ...
    if not np.isfinite(values.to_numpy(dtype=float)).all() or (values <= -1.0).any():
        raise poe.BotError("return must be finite and greater than -1")
...
def _validated_daily_dates(daily: pd.DataFrame) -> pd.DataFrame:
    if "date" not in daily.columns:
        raise poe.BotError("daily data is missing date column")
    out = daily.copy()
    parsed = pd.to_datetime(out["date"], errors="coerce")
    if parsed.isna().any():
        raise poe.BotError("daily data contains invalid date")
    normalized = parsed.dt.normalize()
    duplicates = normalized.duplicated(keep=False)
    if duplicates.any():
        sample = ", ".join(
            pd.Timestamp(value).date().isoformat()
            for value in normalized.loc[duplicates].drop_duplicates().iloc[:6]
        )
        raise poe.BotError(f"duplicate normalized date in daily data: {sample}")
    out["date"] = normalized
    return out.sort_values("date").reset_index(drop=True)


def _validated_daily_frame(daily: pd.DataFrame) -> pd.DataFrame:
    out = _validated_daily_dates(daily)
    for column in ("return", "nav", "wealth"):
        ...
        if not finite.all():
            bad_dates = ", ".join(
                pd.Timestamp(value).date().isoformat()
                for value in out.loc[~finite, "date"].iloc[:6]
            )
            missing_prefix = f"missing {column} inside daily data; " if values.isna().any() else ""
            raise poe.BotError(
                f"{missing_prefix}{column} must be finite in daily data: {bad_dates}"
            )
        if column == "return":
            valid_domain = values > -1.0
            requirement = "greater than -1"
        else:
            valid_domain = values > 0.0
            requirement = "positive"
        if not valid_domain.all():
            bad_dates = ", ".join(
                pd.Timestamp(value).date().isoformat()
                for value in out.loc[~valid_domain, "date"].iloc[:6]
            )
            raise poe.BotError(f"{column} must be {requirement} in daily data: {bad_dates}")
...
def trading_day_window_start(index: pd.Index, end: pd.Timestamp, trading_days: int) -> pd.Timestamp:
    if isinstance(trading_days, (bool, np.bool_)) or int(trading_days) <= 0:
        raise ValueError("trading_days must be a positive integer")
    sessions = pd.DatetimeIndex(pd.to_datetime(index)).normalize().unique().sort_values()
    sessions = sessions[sessions <= pd.Timestamp(end).normalize()]
    if len(sessions) < int(trading_days):
        return pd.NaT
    return pd.Timestamp(sessions[-int(trading_days)])
...
def _default_performance_ranges_for_daily(
    ...
    dates = pd.DatetimeIndex(_validated_daily_dates(daily)["date"])
    ranges: list[tuple[str, pd.Timestamp, pd.Timestamp]] = [
        ("full_sample", pd.Timestamp(earliest).normalize(), pd.Timestamp(latest).normalize())
    ]
    for label in ("10Y", "5Y", "3Y", "1Y"):
        start = trading_day_window_start(dates, latest, MANDATORY_WINDOW_TRADING_DAYS[label])
        ranges.append((label, start, pd.Timestamp(latest).normalize()))
    ranges.append(("from_2020", EVAL_START, pd.Timestamp(latest).normalize()))
    return ranges
...
def resolve_performance_ranges_for_daily(
    ...
    validated = _validated_daily_dates(daily)
    now = _bj_today_naive() if now is None else _normalize_query_date(now)
    latest = (
        pd.Timestamp(validated["date"].iloc[-1]).normalize()
        if latest_date is None
        else _normalize_query_date(latest_date)
    )
    earliest = (
        pd.Timestamp(validated["date"].iloc[0]).normalize()
        if earliest_date is None
        else _normalize_query_date(earliest_date)
    )
    parsed = parse_all_date_ranges(query, now=now)
    ranges = [(f"{s.date()}~{e.date()}", s, e) for s, e in parsed]
    seen = {
        (label, pd.Timestamp(start).normalize(), pd.Timestamp(end).normalize())
        for label, start, end in ranges
    }
    for item in _default_performance_ranges_for_daily(validated, latest, earliest):
        start_key = pd.NaT if pd.isna(item[1]) else pd.Timestamp(item[1]).normalize()
        key = (item[0], start_key, pd.Timestamp(item[2]).normalize())
        if key not in seen:
            ranges.append(item)
            seen.add(key)
    return ranges
...
def _mandatory_window_na_reason(
    ...
    if required_rows is not None and (
        pd.isna(start) or (available_rows is not None and available_rows < required_rows)
    ):
        actual = 0 if available_rows is None else int(available_rows)
        return f"insufficient history: {actual} unique trading days available; {required_rows} required"
...
def _score_red_light_lines(daily: pd.DataFrame) -> list[str]:
    ...
    for item in _signal_rank_rows(daily, limit=len(ASSETS)):
        ...
        code = str(item["code"])
        lines.append(
            f"- 红灯: **{_asset_name(code)}** Raw Score **{_fmt_num(raw_score, 4)}** ≥ {SCORE_MAX:g}，"
            "仅展示，不进入候选池。"
        )
    return lines
...
def _entry_state_text(
    ...
    return "分批建仓已关闭：有效候选一次全额建仓，无候选则现金。"
...
def _format_signal_report_compact(
    ...
    lines: list[str] = []
    lines.append(f"## SubD六ETF V1.3 {mode_label}操作信号")
    lines.append("")
    lines.append(f"信号日: **{sig['date']}** | 数据: **{data_status['label']}** | 来源: **{source_note}**")
    lines.append("仅提供模型信号，实际持仓须自行核对；手动执行，不自动下单。")
    lines.append("")
    lines.append("### 结论")
    lines.append("")
    ...
    lines.append("|:-|--:|:-|")
    lines.append(f"| 基础仓位 | **{_fmt_pct(holding_fraction)}** | V1.3有效候选一次全额建仓，无候选则现金 |")
    lines.append(f"| Target-vol scale(今日已生效) | **{_fmt_num(target_vol_scale_effective, 3)}x** | 用于本日收益 |")
    lines.append(f"| Target-vol scale({target_scale_label}) | **{_fmt_num(target_vol_scale_next, 3)}x** | 目标波动率已关闭，固定1倍scale |")
    lines.append("| Scale调整阈值 | **不适用** | 目标波动率已关闭 |")
    lines.append(f"| 过热防守scale(今日已生效) | **{_fmt_num(overheat_scale_effective, 3)}x** | 用于本日收益 |")
    lines.append(f"| 过热防守scale({target_scale_label}) | **{_fmt_num(overheat_scale_next, 3)}x** | 已关闭，固定1倍scale |")
...
def _momentum_status(score: float, r2: float) -> str:
    ...
    if score <= SCORE_MIN:
        return f"Score≤{SCORE_MIN:g}"
    if score >= SCORE_MAX:
        return f"Score≥{SCORE_MAX:g}"
    return "入选"
...
def format_live_params_snapshot(
    ...
    lines.append("|:-|--:|:-|")
    lines.append(f"| Target-vol scale(今日已生效) | **{_fmt_num(target_vol_scale_effective, 3)}x** | 用于本日收益 |")
    lines.append(f"| Target-vol scale(收盘后目标) | **{_fmt_num(target_vol_scale_next, 3)}x** | 目标波动率已关闭，固定1倍scale |")
    lines.append("| Scale调整阈值 | **不适用** | 目标波动率已关闭 |")
    lines.append(f"| 过热防守scale(今日已生效) | **{_fmt_num(overheat_scale_effective, 3)}x** | 用于本日收益 |")
    lines.append(f"| 过热防守scale(收盘后目标) | **{_fmt_num(overheat_scale_next, 3)}x** | 已关闭，固定1倍scale |")
    lines.append(f"| 执行scale | **{_fmt_num(execution_scale, 3)}x** | Target-vol × 过热防守 |")
    ...
    lines.append(f"| Live price limit by ETF | **{_live_price_limit_summary()}** | {LIVE_PRICE_LIMIT_DESCRIPTION} |")
    ...
    lines.append("")
    lines.append("说明: Score 为25日加权对数斜率年化动量；只有 `0.5 < Score < 5.5` 且 R² 达标的 ETF 才进入候选池。")
...
def render_nav_curve_png(
    ...
    ax_nav.set_title(f"SubD Six ETF V1.3 NAV Curve | {label} | {period_start} to {period_end}", fontsize=13, fontweight="bold")
...
class SubDSixEtfV13Bot:
    ...
    def _handle_params(self, live: bool = False):
        with poe.start_message() as msg:
            ...
            msg.write(f"## SubD六ETF V1.3 {title}\n\n")
            if source_note:
                msg.write(f"数据: {source_note}\n\n")
            msg.write("| 参数 | 当前值 | 说明 |\n|:-|:-|:-|\n")
            msg.write(f"| 版本 | **{VERSION}** | {V13_SCENARIO} |\n")
            msg.write(f"| 起始日期 | **{START_DATE.date()}** | 回测起点 |\n")
            msg.write(f"| 评估起点 | **{EVAL_START.date()}** | 正式统计窗口起点 |\n")
            msg.write(f"| 加权斜率窗口 | **{LOOKBACK}日** | 对数价格加权线性拟合 |\n")
            msg.write(f"| Score入选范围 | **{SCORE_MIN:g} < Score < {SCORE_MAX:g}** | 超过上限或低于下限只显示，不进入候选池 |\n")
            msg.write(f"| R\u00b2门槛 | **{R2_THRESHOLD:.2f}** | score入选过滤 |\n")
            msg.write("| 目标波动率 | **关闭** | 固定1倍，不调用波动率叠加层 |\n")
            msg.write("| 波动率窗口 | **不适用** | 目标波动率已关闭 |\n")
            msg.write(f"| 最大杠杆 | **{DEFAULT_MAX_LEV:.1f}x** | scale上限 |\n")
            msg.write("| Scale调整阈值 | **不适用** | 目标波动率已关闭 |\n")
            msg.write(f"| 切换buffer | **关闭（{SWITCH_BUFFER:.2f}x）** | 直接选择分数最高的合格候选 |\n")
            msg.write(f"| 新资产首笔 | **{INITIAL_ENTRY_FRACTION:.0%}** | 从现金或换新资产时先买入 |\n")
            msg.write("| 分批建仓 | **关闭** | 一次全额建仓，不等待下跌日 |\n")
            msg.write("| MA60过热防守 | **关闭** | 不参与信号或仓位计算 |\n")
            msg.write("| 现金收益 | **0%** | 无合格候选时持有现金 |\n")
            ...
            msg.write("| 数据源 | **AkShare/Eastmoney qfq -> Tencent fqkline qfqday -> Eastmoney HTTP qfq** | 历史回测优先使用前复权/连续日收盘价；Tencent仅接受经验证的qfq payload；原始未复权fallback不得进入正式信号或绩效路径 |\n")
    ...
    def _handle_performance(self, query: str):
        ...
        available_rows = int(pd.to_datetime(daily["date"]).dt.normalize().nunique())
        ...
        with poe.start_message() as msg:
            if chart_range is not None:
                ...
                msg.write("\n")
            msg.write("## SubD六ETF V1.3 表现\n\n")
            msg.write(f"数据: {source_note}\n")
            msg.write("口径：上市后逐步加入的六ETF可用资产池；Full/10Y不等同于固定六ETF共同存续期。仅为策略模型，手动执行，不自动下单。\n")
