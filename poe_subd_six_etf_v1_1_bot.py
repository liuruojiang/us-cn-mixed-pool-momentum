...
def load_close(config: RunConfig) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if diagnostic.any():
        ...
        raise RuntimeError(
            "Raw/unadjusted history is diagnostic-only and cannot enter the formal V1.1 path: "
            + rejected
        )
...
def build_curves(
    ...
    staged = apply_target_vol_overlay(
        ...
        TARGET_VOL, config.vol_window, config.max_lev, config.one_way_cost, price_ffill_flags,
    )
